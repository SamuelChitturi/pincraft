...
from pinapp.models import Image, Comments, Category, Follow,Board
...
def display_saved_images(request):
    ...
    print(boards)
    return render(request, 'saved_images.html', {'saved_images': saved_images,'boards': boards})
...
from .models import Category, SavedImage

def saved_images_board(request):
    # Query all categories
    categories = Category.objects.all()

    # Create a dictionary to store category-wise saved images
    category_saved_images = {}

    # Loop through each category
    for category in categories:
        # Query saved images related to the current category
        saved_images = SavedImage.objects.filter(image__category=category)

        # Add the category and its saved images to the dictionary
        category_saved_images[category] = saved_images

    return render(request, 'saved_images_board.html', {'category_saved_images': category_saved_images})
...
def create_board(request):
    if request.method == 'POST':
        ...
        user = request.user
        new_board = Board.objects.create(name=name, user=user)
        return redirect('pinapp:saved_images_board')  # Redirect to saved images board after creating the board
...
def send_message(request):
    if request.method == 'POST':
        ...
        return redirect('pinapp:chat')
    return redirect('pinapp:base')  # Redirect to home if not a POST request
